...
The data directory is the default location where the sixcoin Core files are stored.
...
Windows  | `%APPDATA%\sixcoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `sixcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `sixcoin.conf`       | User-defined configuration settings for `sixcoind` or `sixcoin-qt`; can be specified by `-conf` option
`./`               | `sixcoind.pid`        | Stores the process ID (PID) of `sixcoind` or `sixcoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `sixcoind` or `sixcoin-qt`
...
`sixcoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by sixcoin Core:
