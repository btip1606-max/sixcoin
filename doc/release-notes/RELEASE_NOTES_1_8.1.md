...
sixcoin Core 1.8.1 is primarily a bugfix release, bringing sixcoin Core in
line with Bitcoin 0.9.3. sixcoin Core 1.8.1 also adds in support for printing
...
## sixcoin Core Release Notes

* Translation improvements and corrections
* Disable Transifex based translation process (not used by sixcoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 sixcoin Core clients
* Use HTTPS with SIXchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/sixcoin/sixcoin/issues/134).
...
This only affects values significantly below 1 SIX, however merchants may wish to round to
the nearest SIX on all transactions for now.
