sixcoin Core version 1.14.6 is now available from:

  <https://github.com/sixcoin/sixcoin/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All sixcoin Core users - miners, services,
...
sixcoin network from 1 SIX to 0.01 SIX. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 SIX as
   change if possible.
2. If after signing there is more than 0.01 SIX left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 SIX, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 SIX will not be accepted as valid by default.
...
```
sixcoin-cli addnode <address> remove
```
...
```
sixcoin-cli liststucktransactions
```
...
```
sixcoin-cli setmaxconnections <desired_amount>
```
...
```
sixcoin-cli rescan <start_block>
```
...
efforts to make sixcoin more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by sixcoin Core.
