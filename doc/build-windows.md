...
Below are some notes on how to build sixcoin Core for Windows.
...
To build sixcoin from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the sixcoin GitHub repository](https://github.com/sixcoin/sixcoin/)).
...
Note that for WSL v1 the sixcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/sixcoin, AND not under, for example, /mnt/d/sixcoin.
...
```
Ensure that this toolchain can build for `posix` (else you may have compilation errors for sixcoin's dependencies):
```bash
...
way. This will install to `c:\workspace\sixcoin`, for example:
```bash
make install DESTDIR=/mnt/c/workspace/sixcoin
```
